...
public class ProjectService {
     ...
     */
    public ProjectService(ProjectRepository projectRepository, UserRepository usersRepository, TeamRepository teamsRepository, ArtifactRepository artifactsRepository, TeamService teamService, UserRepository userRepository) {
    ...
    public ResponseEntity<?> createProject(ProjectCreateDto projectCreateDto) {
        ...
        try {
            ...
            TeamCreateDto teamCreateDto = new TeamCreateDto("Default team", "Default team for project " + project.getProjectName(), project.getUsers());
    ...
    private void addProjectsToUsers(Project project) throws Exception {
        ...
        for (String userId : userList) {
            ...
            String emailMessage = String.format("""
                    ...
                    Best regards,
                    %s
                    """, user.getUsername(), project.getProjectName(), project.getProjectDescription(), owner.getName() + " " + owner.getSurname());
    ...
    public ResponseEntity<?> addArtifact(String projectId, String artifactId) {
        ...
        if (artifactId == null || artifactId.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Artifact ID cannot be null or empty");
        }
    ...
    public ResponseEntity<?> closeProject(String projectId) {
        ...
        String currentUserId = getLoggedInUserId();
        if (!project.getOwnerId().equals(currentUserId)) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Only the project owner can close the project!");
        }
