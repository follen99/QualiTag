...
public class UserServiceTest {
  ...
  public void setUp() {
    ...
    owner = new User("owner", "owner@example.com", "password2", "Alice", "Smith");
    ...
    user1.setTagIds(
        new ArrayList<>(Arrays.asList("6744ba6c60e0564864250e89", "6755b79afc22f97c06a34275")));
    ...
    // Initialize the dto
    userModifyDto = new UserModifyDto("user1Figo", "user1Figo@example.com", "Jane", "Doe");
    passwordUpdateDto = new PasswordUpdateDto("pAssword12$", "pAssword12$");
   ...
   */
  @Test
  void testGetUserTagsUnauthorized() {
    // Arrange
    Authentication authentication = mock(Authentication.class);
    when(authentication.isAuthenticated()).thenReturn(true);
   ...
   */
  @Test
  void testGetUserTagsUserNotFound() {
    // Arrange
    when(userRepository.findByUsername("user1")).thenReturn(null);
  ...
  void testUpdateUserSuccess() {
    ...
    // Assert
    verify(jwtService, times(1)).generateToken(new CustomUserDetails(user1));
   ...
   */
  @Test
  void testUpdateUserNoChange() {
    // Arrange
    UserModifyDto noChangeUserModifyDto =
        new UserModifyDto("user1", "user1@example.com", "Jane", "Doe");
    when(userRepository.findByUsername("user1")).thenReturn(user1);
    when(userRepository.save(user1)).thenReturn(user1);
    when(userRepository.existsByUsername("user1Figo")).thenReturn(false);
    when(userRepository.existsByEmail("user1Figo@example.com")).thenReturn(false);
    ...
    verify(userMapper, times(1)).updateEntity(noChangeUserModifyDto, user1);
    verify(userRepository, times(1)).save(user1);
    Map<String, String> responseBody = new HashMap<>();
    responseBody.put("msg", "User updated successfully.");
    responseBody.put("wholeuser", null);
   ...
   */
  @Test
  void testUpdateUserMissingFields() {
    // Arrange
    UserModifyDto invalidUserModifyDto =
        new UserModifyDto("user1Figo", "user1Figo@exaple.com", "", "");
    when(userRepository.findByUsername("user1")).thenReturn(user1);
    when(userRepository.existsByUsername("user1Figo")).thenReturn(false);
    when(userRepository.existsByEmail("user1Figo@example.com")).thenReturn(false);
    when(userRepository.save(user1)).thenReturn(user1);
   ...
   */
  @Test
  void testUpdateUserInvalidUsername() {
    // Arrange
    UserModifyDto invalidUserModifyDto =
        new UserModifyDto("ab", "user1Figo@example.com", "Jane", "Doe");
    when(userRepository.findByUsername("user1")).thenReturn(user1);
  ...
  void testUpdateUserInvalidEmail() {
    // Arrange
    UserModifyDto invalidUserModifyDto = new UserModifyDto("user1Figo", "user1Figo", "Jane", "Doe");
    when(userRepository.findByUsername("user1")).thenReturn(user1);
    when(userRepository.existsByUsername("user1Figo")).thenReturn(false);
  ...
  void testUpdatePassword() {
    ...
    // Act
    ResponseEntity<?> response = userService.updatePassword(passwordUpdateDto, "user1");
  ...
  void testUpdatePasswordMissingFields() {
    ...
    // Act
    ResponseEntity<?> response = userService.updatePassword(invalidPasswordUpdateDto, "user1");
   ...
   */
  @Test
  void testUpdatePasswordPasswordsDontMatch() {
    // Arrange
    PasswordUpdateDto invalidPasswordUpdateDto =
        new PasswordUpdateDto("pAssword12$", "pAssword12$1");
   ...
   */
  @Test
  void testDeleteUserUserIsOwner() {
    // Authentication
    Authentication authentication = mock(Authentication.class);
    when(authentication.isAuthenticated()).thenReturn(true);
